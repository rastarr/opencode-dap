...
- **Breakpoints**: `set_breakpoint`/`remove_breakpoint` with source (`file`+`line`) or function (`function`); optional `condition` for conditional breakpoints.
  `set_function_breakpoint`/`remove_function_breakpoint` for explicit function breakpoints.
...
- Timeouts apply per-request, not to the full session lifetime. Default timeout is 30s, clamped to 5-300s.
</instruction>
